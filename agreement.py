...
def N_phrase_num(tr):
    ...
    if (tr.label() == 'N'):
        ...
        if (len(tr)==1): #  AN[x]  -> N[x]
            return N_phrase_num(tr[0])
        else:  # AN[x]  ->  A AN[x] hence take the second tree
            return N_phrase_num((tr[1]))
    elif (tr.label()=="NP"):
        if (len(tr)==1):
            return N_phrase_num(tr[0]) #might be either P or Nom
        else: #NP[s]  -> AR Nom[s] hence take the second tree
            return N_phrase_num((tr[1]))
    elif (tr.label()=="P"):
        return 's'  #  NP[s]  -> P
    else:
...
def V_phrase_num(tr):
    ...
    if (tr.label() == 'T' or tr.label() == 'I'):
        ...
        return tr[0][2] # return s or p for DOs and DOp, BEs and BEp
    elif (tr.label()== "Rel"):
        ...
        if (len(tr)==1):
            ...
            if (V_phrase_num(tr[0])== V_phrase_num(tr[2])): # if  QP[x]  -> DO[y] NP[y] T[p] then check that DO and NP match
                return V_phrase_num(tr[0])
            else:
                return ""
    else:
...
def check_node(tr):
    ...
    elif (rule == 'NP -> AR Nom'): # N P[s]  -> AR Nom[s]
        return (N_phrase_num(tr[1]) == 's')
    elif (rule=='NP -> Nom' ): #NP[p]  -> Nom[p]
        return  (N_phrase_num(tr[0])=='p')
    elif (rule =='QP -> DO NP T'): # QP[x]  -> DO[y] NP[y] T[p]
        return (matches(N_phrase_num(tr[1]), V_phrase_num(tr[0])) and V_phrase_num(tr[2])=='p')
    elif (rule == 'VP -> BE NP'):
...
if __name__ == "__main__":
    ...
    #  tr = restore_words(tr0,['Who','likes','John','?'])
    #  tr.draw()
    # tr1=all_valid_parses(lx,['Who','does','Mary','like','who','hates','John','?'])[0]
    # tr=restore_words(tr1,['Who','does','Mary','like','who','hates','John','?'])
    # tr.draw()
    # tr2=all_valid_parses(lx,['Which','ducks','does','John','like','?'])[0]
